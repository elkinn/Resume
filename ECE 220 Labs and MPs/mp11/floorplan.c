...
int is_internal_node(node_t* ptr) {
  // TODO: (remember to modify the return value appropriately)
  if(ptr->left != NULL || ptr->right != NULL){ //if the node has nodes to the left and right, it is internal
    return 1;
  }
  return 0; //otherwise, it's external
}
...
int is_in_subtree(node_t* a, node_t* b) {
  // TODO: (remember to modify the return value appropriately)
  while(b != NULL){ //traverse up the subtree b is in and look for a
    if(a == b){
      return 1;
    }
    b = b->parent;
...
void rotate(node_t* ptr) {
  // TODO:
  int temp;
  temp = ptr->module->w; //create temp variable to hold width
  ptr->module->w = ptr->module->h; //set width equal to height
  ptr->module->h = temp; //set height equal to temp width
  return;
}
...
void recut(node_t* ptr) {
  ...
  // TODO:
  if(ptr->cutline == V){ //if it's vertical, set it to horizontal
    ptr->cutline = H;
  }
  else if(ptr->cutline == H){ //if horizontal, set to vertical
    ptr->cutline = V;
  }
  return;
}
...
void swap_module(node_t* a, node_t* b) {
  ...
  // TODO:
  module_t *aMod = a->module; //create temp value to hold a's module
  a->module = b->module; //move b's module to a
  b->module = aMod; //move a's module to b
...
void swap_topology(node_t* a, node_t* b) {
  ...
  // TODO:
  node_t *aParent, *bParent; //create temp node pointers to hold the parents of a and b
  aParent = a->parent;
  bParent = b->parent;

  if(a->parent->left == a){ //if a is to the left of its parent, set that node to b
    a->parent->left = b;
  }
  else if(a->parent->right == a){ //if a is to the right, set that node
    a->parent->right = b;
  }
  if(b->parent->left == b){ //repeat this process with b
    b->parent->left = a;
  }
  ...
  a->parent = bParent; // correct parent nodes
  b->parent = aParent;
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
  ...
  // TODO
  postfix_traversal(ptr->left, nth, expression); //traverse down the left of the tree first recursively
  postfix_traversal(ptr->right, nth, expression); //then right
  expression[*nth].module = ptr->module; //set expression's module to ptr's module. if the ptr is NULL, this transfers
  expression[*nth].cutline = ptr->cutline; //do the same with cutline. if cutline is UNDEFINED_CUTLINE, this transfers
  *nth += 1; //increment the value of nth so by the time you reach the top, nth = N-1
...
{
  // TODO: (this algorithm assumes every internal node has a left and right node)
  int sum =  0; //initialize holder for sum
  if(is_internal_node(ptr)){ //if the node is internal, recursively add to the value of sum the resources of its left and right nodes
    sum += get_total_resource(ptr->left);
    sum += get_total_resource(ptr->right);
    return sum;
  }
  else{ //base case, return resources of leaf node
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  node_t *ptr = (node_t *)malloc(sizeof(node_t)); //create a new node_t
  ptr->parent = par; //define ptr's parent as the root passed to this recursive call
  if(n == num_modules - 1){ //base case: assign data to the modules pointed by ptr
    ptr->module = modules + n;
    ptr->cutline = UNDEFINED_CUTLINE;
    return ptr;
  }
  ptr->cutline = V; //make ptr internal node with vertical cutline
  ptr->module = NULL;
  ptr->right = (node_t *)malloc(sizeof(node_t)); //create node_t for right child, assign data to the module from ptr->right
  ptr->right->module = modules + n;
  ptr->right->cutline = UNDEFINED_CUTLINE;
  ptr->left = init_slicing_tree(ptr, n+1); //define the left child recursively
  return ptr; //return the ptr
}
